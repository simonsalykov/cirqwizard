...
public class ApertureMacro extends Aperture
{
    ...
    @Override
    public com.vividsolutions.jts.geom.Polygon createPolygon(int x, int y, int inflation)
    {
        return (com.vividsolutions.jts.geom.Polygon) VectorToolPathGenerator.factory.createGeometryCollection(primitives.stream().
                map(p -> p.createPolygon(x, y, inflation)).toArray(com.vividsolutions.jts.geom.Polygon[]::new)).buffer(0);
    }
